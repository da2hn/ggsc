...
public class ResultStatsController {
	...
	@RequestMapping(value = "/mainApplPbCnsStateList_json.do", method = RequestMethod.POST)
	public String mainApplPbCnsStateList_json(HttpServletRequest request, ModelMap model, @RequestParam Map<String,Object> map) {
		// 권한 관리 시작
		EgovMap loginVo = (EgovMap) request.getSession().getAttribute("LoginVO");
		...
		if (userAuth > 1) { // 센터 검색 권한이 없으면
			map.put("schCenterGb",(Integer.toString(userCenterGb)));
		}
		
		Map<String, Object> map2 = new HashMap<String, Object>();
		List<Map<String, Object>> map4 = new ArrayList<Map<String, Object>>();
			
		map2.put("hclassCd", "G58");
		
		List<EgovMap> list = resultStatsService.getMainApplPbCnsStateList(map);
		
		for(int a=0; a < list.size(); a++) {
			map2.put("mclassCd", list.get(a).get("mclassCd"));
			Map<String, Object> map3 = new HashMap<String, Object>();
			List<EgovMap> applList = adminManageService.getGroupMngSList(map2);
			for(int b=0; b < applList.size(); b++) {
				map3.put(String.valueOf(b),applList.get(b).get("sclassNm"));
			}
			map4.add(map3);
