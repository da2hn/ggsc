...
// 행정지원-검사지관리-검사지원본 다운로드
function fn_pdfDownload2(number){
	if(Number(number).toString() != 'NaN'){
	       $("#tmpFrame").remove();
	        $('body').append("<iframe id='tmpFrame' name='tmpFrame' style='display:none'></iframe>");
			var frm = document.createElement('form');
			frm.method = 'post';
			frm.action = 'pdfDownload2.do?test=0';
			frm.target = 'tmpFrame';
			frm.name='pdfForm';
			frm.style.display = 'none';
			frm.appendChild($("input[name='_csrf']")[0]);
...
function fn_excelDownload(number, schForm){
	...
	if(schForm){
		...
		document.tmpForm.schPrevPostGb.value = schForm.schPrevPostGb ?  schForm.schPrevPostGb.value : ''; // 사전사후검사구분
