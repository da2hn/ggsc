...
<script type="text/javascript">
	...
	function fn_reg(mnuCd){
		document.location.href = "/gnoincoundb/youtubeForm.do?mnuCd=" + mnuCd;
	}
	
	function fn_detail(num){
		document.location.href = "/gnoincoundb/youtubeDtl.do?num=" + num + "&mnuCd=${mnuCd}";
	}

	function fn_search(mnuCd) {
		document.searchForm.action = "/gnoincoundb/youtubeList.do?mnuCd=" + mnuCd;
       	document.searchForm.submit(); 
	}
	
	function list(curPage) {
		$("#currentPageNo").val(curPage);
		document.searchForm.action = "/gnoincoundb/youtubeList.do";
       	document.searchForm.submit();
	}
...
<section id="content">
	<h2 class="h2-title"><i class="fa fa-check-square"></i>유튜브</h2>
				...
				<div class="search-group">
					...
					<span class="label">
						<select id="schType" name="schType" style="margin-left: 20px; font-size: 12px;">
							<option value="1" <c:if test="${vo.schType==1}">selected</c:if>>제목</option>
			...
			<table class="table-style1" style="margin-bottom: 5px;">
				...
				<thead>
					<tr>
						<th scope="col">번호</th>
						<th scope="col">등록일</th>
						<th scope="col">유튜브제목</th> 
						<th scope="col">첨부파일</th>
						<th scope="col">등록자</th>
					</tr>
				</thead>
				<tbody id="tby1">
					<c:if test="${youtubeList.size() == 0 }">
						<tr>
							<td colspan="5">정보가 없습니다.</td>
						</tr>
					</c:if>
					<c:if test="${youtubeList.size() > 0 }">
						<c:forEach items="${youtubeList }" var="result">
							<tr>
