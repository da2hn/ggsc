...
<script type="text/javascript">
		...
		*/
		if(param == "S") {
			if(confirm("등록 하시겠습니까?")){
				$("#num").val(0);
				document.frm.action = "/gnoincoundb/popupReg.do?save=" +param;
		       	document.frm.submit();
			}
		} else if(param == "U") {
	...
	function fn_list(mnuCd){
		document.location.href = "/gnoincoundb/popupList.do?mnuCd=" + mnuCd;
	}  	
...
<section id="content">
	<h2 class="h2-title"><i class="fa fa-check-square"></i>
		<c:if test="${empty detail.num }">팝업 등록</c:if>
		<c:if test="${!empty detail.num }">팝업 상세 및 수정</c:if>
	</h2>
	...
	<div class="btn" style="text-align: right; display: block;">
		...
		<c:if test="${ detail != null && (authCd <= 1 || ( authCd > 1 && userId == detail.regId )) }">
			<button type="button" class="btn-basic"  onClick="javascript:fn_reg('D');">삭제</button>
		</c:if>
