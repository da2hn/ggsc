...
<script type="text/javascript">
	...
	function fn_reg(mnuCd){
		document.location.href = "/gnoincoundb/popupDtl.do?mnuCd=" + mnuCd;
	}
	
	function fn_detail(num){
		document.location.href = "/gnoincoundb/popupDtl.do?num=" + num + "&mnuCd=${mnuCd}";
	}

	function fn_search(mnuCd) {
		document.searchForm.action = "/gnoincoundb/popupList.do?mnuCd=" + mnuCd;
       	document.searchForm.submit(); 
	}
	
	function list(curPage) {
		$("#currentPageNo").val(curPage);
		document.searchForm.action = "/gnoincoundb/popupList.do";
       	document.searchForm.submit();
	}
...
<section id="content">
	<h2 class="h2-title"><i class="fa fa-check-square"></i>팝업관리</h2>
		...
		<form name="frm" id="frm" method="post" onsubmit="return false">
			...
			<table class="table-style1" style="margin-bottom: 5px;">
				...
				<tbody id="tby1">
					...
					<c:if test="${popupList.size() > 0 }">
						<c:forEach items="${popupList }" var="result">
							<tr>
